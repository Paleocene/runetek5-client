...
public abstract class Class90 implements Interface13 {
	...
	public final void method7749() {
		...
		if (Static588.getCurrentTimeWithDrift() - this.aLong274 > 10000L) {
		...
		if (local9 == this.anInt8730 && local9 != 0) {
			...
			if (local9 < local40) {
				...
				if (local55 > 0L) {
					...
					@Pc(81) long local81 = (Static588.getCurrentTimeWithDrift() - this.aLong274) * 10000L;
					if (local81 < local72) {
			...
			this.aLong274 = Static588.getCurrentTimeWithDrift();
		}
		return local13;
	}
	...
	public boolean method7747() {
		...
		if (!this.aJs5109.isFileReady(this.aClass138_5.anInt4415)) {
			local5 = false;
		}
		if (!this.aJs5110.isFileReady(this.aClass138_5.anInt4415)) {
			local5 = false;
		}
		return local5;
	}
