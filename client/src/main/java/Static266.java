		...
		for (@Pc(312) Linkable_Sub35 local312 = (Linkable_Sub35) Static391.aLinkedList34.head(); local312 != null; local312 = (Linkable_Sub35) Static391.aLinkedList34.next()) {
			...
			for (local107 = local93 >>> 3; local107 <= local101; local107++) {
				label82: for (local110 = local98 >>> 3; local110 <= local104; local110++) {
					...
					Static267.anIntArray329[local22] = client.maps.getGroupId("m" + local107 + "_" + local110);
					anIntArray615[local22] = client.maps.getGroupId("l" + local107 + "_" + local110);
					Static68.anIntArray316[local22] = client.maps.getGroupId("um" + local107 + "_" + local110);
					Static298.anIntArray367[local22] = client.maps.getGroupId("ul" + local107 + "_" + local110);
					local22++;
				}
			}
		}
